...
BNTcandles::BNTcandles(QWidget *parent) : QMainWindow(parent), ui(new Ui::BNTcandles)
{
    ...
    db.setPort(3306);
    db.setDatabaseName("bntcandles");
    db.setUserName("root");
    db.setPassword("darkshadows"); */
    ...
    // Set up database connection
    QSqlDatabase db = QSqlDatabase::addDatabase("QMYSQL");    db.setHostName(host);
    db.setPort(port);
    ...
    if (!customerModel->select()) {
        qDebug() << "Customer model error:" << customerModel->lastError().text();
    ...
    if (!productModel->select()) {
        qDebug() << "Product model error:" << customerModel->lastError().text();
    }
...
{
    AddOrderWindow dialog(db, this);
    if (dialog.exec() == QDialog::Accepted) {
        if (dialog.customerId() == 0) {
            QMessageBox::warning(this, "Input Error", "Please select a customer.");
            return;
        }
        if (dialog.totalAmount() <= 0) {
            QMessageBox::warning(this, "Input Error", "Total amount must be positive.");
            return;
        }

        QSqlQuery query(db);
        query.prepare("INSERT INTO `orders` (customer_ID, order_date, total_amount, status) "
                      "VALUES (:customerId, :orderDate, :totalAmount, :status)");
        query.bindValue(":customerId", dialog.customerId());
        query.bindValue(":orderDate", dialog.orderDate());
        query.bindValue(":totalAmount", dialog.totalAmount());
        query.bindValue(":status", dialog.status());

        if (query.exec()) {
            int orderId = query.lastInsertId().toInt();
            orderModel->select();  // Fixed: Refresh the table model
            ui->orderTable->resizeColumnsToContents();
            qDebug() << "Order added successfully.";

            // Prompt to add details
            int reply = QMessageBox::question(this, "Add Details", "Do you want to add order details now?", QMessageBox::Yes | QMessageBox::No);
            if (reply == QMessageBox::Yes) {
                OrderDetailsWindow *detailsDialog = new OrderDetailsWindow(QString::number(orderId), db, this);
                detailsDialog->exec();
                delete detailsDialog;
                orderModel->select();  // Refresh again in case details updated totals
            }
        } else {
...
void BNTcandles::editOrder()
{
    ...
    QSqlQuery query(db);
    query.prepare("SELECT customer_ID, order_date, grand_total, status FROM `orders` WHERE order_ID = :orderId");
    query.bindValue(":orderId", orderId);
    if (!query.exec() || !query.next()) {
        QMessageBox::warning(this, "Database Error", "Failed to load order data: " + query.lastError().text());
        return;
    }

    AddOrderWindow dialog(db, this);
    dialog.setWindowTitle("Edit Order");
    dialog.setCustomerId(query.value("customer_ID").toInt());
    dialog.setOrderDate(query.value("order_date").toDate());
    dialog.setTotalAmount(query.value("grand_total").toDouble());
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        query.prepare("UPDATE `orders` SET customer_ID = :customerId, order_date = :orderDate, "
        ...
        if (query.exec()) {
            ...
            if (reply == QMessageBox::Yes) {
                ...
                detailsDialog->exec();
                delete detailsDialog;
                orderModel->select();  // Refresh again in case details updated
            }
        } else {
